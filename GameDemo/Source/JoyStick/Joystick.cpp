...
bool Joystick::init()
{
    ...
    listener->onTouchesMoved = AX_CALLBACK_2(Joystick::onTouchesMoved, this);
    listener->onTouchesBegan = AX_CALLBACK_2(Joystick::onTouchesBegan, this);
    listener->onTouchesEnded = AX_CALLBACK_2(Joystick::onTouchesEnded, this);
...
void Joystick::joystickBoundaryUpdate()
{
    ...
    {
        float angle = -(_joystickSprite->getPosition() - _mainPosition).getAngle() + AX_DEGREES_TO_RADIANS(90);
        _joystickSprite->setPosition(Point(_mainPosition.x + BOUNDARY_DISTANCE * sinf((angle)),
                                           _mainPosition.y + BOUNDARY_DISTANCE * cosf((angle))));
    }
}
...
float Joystick::getDirection() const
{
    if (_isActive)
    {
        return 180 - AX_RADIANS_TO_DEGREES((-_joystickSprite->getPosition() + _mainPosition).getAngle());
    }
    else
    {
        return 0.0f;
